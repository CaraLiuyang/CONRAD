...
SimpleMatrix Sinv = new SimpleMatrix( svd.getS().getRows(),
svd.getS().getCols());
Sinv.zeros();
...
SimpleMatrix tempInv = SimpleOperators.multiplyMatrixProd(svd.getV(),
svd.getreciprocalS());
SimpleMatrix Ainv = SimpleOperators.multiplyMatrixProd(tempInv,
svd.getU().transposed());
...
SimpleOperators.multiplyMatrixProd(svd.getU(), Slowrank);
SimpleMatrix Alowrank = SimpleOperators.multiplyMatrixProd(templowrank,
svd.getV().transposed());
...
System.out.println("Modification of x " +
SimpleOperators.divideElementWise(xn, x).toString());
}
...
SimpleMatrix Slowrank = new SimpleMatrix(svd.getS().getRows(),
svd.getS().getCols());
Slowrank.zeros();
...
SimpleMatrix temp = SimpleOperators.multiplyMatrixProd(svd.getU(),
Slowrank);
SimpleMatrix A0 = SimpleOperators.multiplyMatrixProd(temp,
svd.getV().transposed());
...
SimpleMatrix Iapprox = SimpleOperators.multiplyOuterProd(us,
svd.getV().getCol(k));
...
{
imageRank.setAtIndex(j, i, imageRanks.getAtIndex(j, i, k-1)+(float)
Iapprox.getElement(i, j));
}
...
Grid2D logFFT = new Grid2D(imageC.getMagnSubGrid(0, 0,
imageC.getWidth(), imageC.getHeight()));
...
String filename =
"/proj/i5dmip/vy11pyce/Reconstruction/CONRAD/src/edu/stanford/rsl/tutorial/dmip/yu_fill.jpg";
Grid2D image =
...
Grid3D sheppLoganVolume =
ImageUtil.wrapImagePlus(IJ.openImage(filenameShepp));
