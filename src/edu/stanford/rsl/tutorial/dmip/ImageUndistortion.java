...
String filename ="/proj/i5dmip/ap97alir/Reconstruction/CONRAD/src/edu/stanford/rsl/tutorial/dmip/frame32.jpg";

if(caseNo == 0)
{
filename ="/proj/i5dmip/ap97alir/Reconstruction/CONRAD/src/edu/stanford/rsl/tutorial/dmip/frame32.jpg";
}
else if(caseNo == 1)
{
filename ="/proj/i5dmip/ap97alir/Reconstruction/CONRAD/src/edu/stanford/rsl/tutorial/dmip/undistorted.jpg";
}else if(caseNo == 2)
{
//filename ="C:/StanfordRepo/CONRAD/src/edu/stanford/rsl/tutorial/dmip/frame90.jpg";
filename ="/proj/i5dmip/ap97alir/Reconstruction/CONRAD/src/edu/stanford/rsl/tutorial/dmip/frame90.jpg";
}

Grid2D image =
ImageUtil.wrapImagePlus(IJ.openImage(filename)).getSubGrid(0);
...
{
float r = (float) (d * Math.sqrt( a * Math.pow(((X.getAtIndex(i, j) -
half) / imSize) , 2)  + b * Math.pow(((Y.getAtIndex(i, j) - half) /
imSize) , 2) ) ) ;
R.setAtIndex(i, j, r);
}
}
...
for(int j = 0; j < R.getHeight(); j++)
{
float distortedValue = InterpolationOperators.interpolateLinear(image,
Xd.getAtIndex(i, j), Yd.getAtIndex(i, j));
...
Xu2.setElementValue(j, i, X.getAtIndex((int)((i+1)*fy),
(int)((j+1)*fx)));
Yu2.setElementValue(j, i, Y.getAtIndex((int)((i+1)*fy),
(int)((j+1)*fx)));
Xd2.setElementValue(j, i, Xd.getAtIndex((int)((i+1)*fy),
(int)((j+1)*fx)));
Yd2.setElementValue(j, i, Yd.getAtIndex((int)((i+1)*fy),
(int)((j+1)*fx)));
}
}
...
Math.pow(Xu2_vec.getElement(r),i)*Math.pow(Yu2_vec.getElement(r),j));
cc++;
}
}
}
...
{
xDist.setAtIndex(x,y,(float)(xDist.getAtIndex(x,
y)+u_vec.getElement(cc)*Math.pow(x, k)*Math.pow(y, l)));
yDist.setAtIndex(x,y,(float)(yDist.getAtIndex(x,
y)+v_vec.getElement(cc)*Math.pow(x, k)*Math.pow(y, l)));
cc++;
}
}
}
...
Grid2D differenceImage = (Grid2D)
NumericPointwiseOperators.subtractedBy(quadraticImage,
undistortedImage);
